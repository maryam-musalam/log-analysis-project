...
def most_popular_authors():
    result = query_connect(
          "SELECT authors.name ,COUNT(*) AS views FROM authors JOIN articles "
          "ON authors.id = articles.author JOIN log ON log.path LIKE "
          "CONCAT('%', articles.slug, '%') GROUP BY authors.name ORDER "
          "BY views DESC")
...
def request_lead_to_more_than_one_persentage_error():
    result = query_connect(
           "select error_precentage.date ,error_precentage.error from "
